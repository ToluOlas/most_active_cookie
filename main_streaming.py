...
def stream_rows(file_name: str) -> Iterable[dict[str, str]]: #from list to Iterable (streaming)
    """CSV file to Iterable conversion.

    Primarily for catching potential errors related to finding and
    accessing the log file.
    Generator provides line by line instead of sharing the entire file at once.
    """
    try:
        with open(file_name, newline="", encoding="utf-8-sig") as file:
            yield from csv.DictReader(file) #DictReader provides lazy file reading, line by line
...
def find_most_active_cookies(rows: Iterable[dict[str, str]], target_date: date) -> list[str]:
    ...
    This function relies on a generator.
    """
    counts = Counter()
    ...
    for index, row in enumerate(rows, start=1):
        try:
            ...
            cookie = row["cookie"]
            #print(f"Cookie: {cookie} | Date: {row_date}")
