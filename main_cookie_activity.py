...
def find_single_cookie_activity(rows: list[dict[str, str]], target_date: date | None, target_cookie: str) -> int:
    """Find the activity level of a specific cookie for a given UTC date."""

    cookie_count: int = 0

    for index, row in enumerate(rows, start=1):
        try:
            row_date = datetime.fromisoformat(row["timestamp"]).astimezone(timezone.utc).date()
            cookie = row["cookie"]
        except (ValueError, KeyError):
            logger.warning(f"Skipping invalid row {index}: {row}")
            continue

        if (row_date == target_date or target_date is None) and cookie == target_cookie:
            cookie_count += 1
...
def main() -> None:
    ...
    Modes: find most active cookie | find single cookie activity
    FMAC - Requires a date
    FSCA - Does not require a date, will return activity across the file in no date is given"""
    ...
    parser.add_argument("-d", required=False, help="Date to query, YYYY-MM-DD format.")
    parser.add_argument("--cookie", required=False, help="A specific cookie.")
    ...
    if args.cookie:
        if args.d:
            ...
            result = find_single_cookie_activity(rows, target, args.cookie)
            print(result)
        else:
            result = find_single_cookie_activity(rows, args.d, args.cookie)
            print(result)
    else:
        target = parse_date(args.d)
